...
The only point, that can be considered good or bad depending on the point of view, is that in this case the site will be generated by another instance of Jekyll managed by GitHub, so you will have to check that everything is fine and corresponds to the output of you local Jekyll instance.
