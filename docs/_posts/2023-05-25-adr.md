...
There is almost never one single, optimal solution. Instead, there are many reasonable solutions, no true/false, only better/worse options.  
...
Writing down architectural decisions is the first fundamental step to make them first-class entities in the teamwork final result.  
...
As a software architect, or as a technical leader, or even as a team with a shared leadership, one of your most important responsibilities is taking these decisions. Not writing them down could dangerously suggest that things are happening without a reason, driven by pure instinct or worse randomly.
